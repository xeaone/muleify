...
module.exports = function (path, input, output) {
	var parsedPath = Utility.parsePath(path, input, output);
	var encoding = 'binary';
	...
	return Promise.resolve().then(function () {
		...
		if (parsedPath.extensions.indexOf('l') !== -1) {
			return null;
		} else {
			return Fsep.outputFile(parsedPath.output, text, encoding);
		}
