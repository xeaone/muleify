...
function handleImportPartials (text, directory) {
	...
	partialComments.forEach(function (partialComment) {
		...
		// if exists then use otherwise look in the input
		var partial = Fs.existsSync(path) ? Fs.readFileSync(path, 'binary') : Fs.readFileSync(Path.join(Globals.input, comment.value), 'binary');
