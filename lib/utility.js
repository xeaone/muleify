...
module.exports.parsePath = function (path, input, output) {
	var data = {};

	if (!input) input = process.cwd();
	if (!output) output = process.cwd();
	...
	if (data.isAbsolute) {
		data.absolute = path;
		data.relative = Path.relative(input, path);
	}

	if (data.isRelative)  {
		data.relative = path;
		data.absolute = Path.resolve(input, path);
	}

	data.input = Path.join(input, data.relative);
	data.output = Path.resolve(output, data.relative);
...
module.exports.createSitemap = function (data, domain) {
	...
	text.middle = data.join('</loc></url>\n\t\t<url><loc>');
	text.middle = '<url><loc>' + text.middle + '</loc></url>';
